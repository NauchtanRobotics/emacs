...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Note, use the following in the scratch buffer to test fonts.
...
#+BEGIN_SRC emacs-lisp
    ...
    (global-set-key (kbd "C-c w 6") 'my-window-split-6)
#+END_SRC
...
The irony package is responsible for code completion using libclang.

Note, this package hangs a lot so I'm disabling it.
...
company-mode, modular in-buffer completion framework for emacs. The name stands
for "complete anything". It uses pluggable back-ends and front-ends to retrieve
